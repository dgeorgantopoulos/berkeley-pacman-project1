...
class CornersProblem(search.SearchProblem):
    ...
    def isGoalState(self, state):
        ...
        "*** YOUR CODE HERE ***"
        count=len(state[1])
        if(count==4):    #theloume na exei paei sta 4 corner tou map
            print("you have reached all corners.")
            return 1
        return 0
    ...
    def getNextState(self, state, action):
        ...
        "*** YOUR CODE HERE ***"
        li=list()
        li.extend(state[1]) #kathe fora dhmiourgw neo list, kai tou vazw ta hdh visited corners
        if ((nextx,nexty) in self.corners): #an to next state einai corner
            if ((nextx,nexty) not in li):
                li.append((nextx,nexty))  #valto sth lista li (sth opoia tha  yparxoun kai alla corners an exoun hdh episkefthei)
        return ((nextx,nexty), tuple(li))   #kane return tis syntetagmenes kai ws tuple ta corners pou exeis episkefthei (theloyme tuple gia na einai hashable)
...
def cornersHeuristic(state, problem):
    ...
    if problem.isGoalState(state):
        return 0#an eimaste hdh se ena corner to kostos metakinisis se ayto einai 0
    li=list()
    visited=state[1]    #ta corners pou exoume hdh episkeftei
    for c in corners:
        if c not in visited:
            li.append(util.manhattanDistance(state[0],c))  #ypologismis manhattan aptis twrines syntetagmenes mas gia tis syntetagmenes tou corner
    li.sort(reverse=True)  #sortarw list gia na vrw thn mikroterh dynath
    return li[0]
...
def foodHeuristic(state, problem):
    ...
    "*** YOUR CODE HERE ***"
    maxi=0
    lis2=foodGrid.asList()    #lista apo syntetagmenes 
    if len(lis2)==0:
        return 0
    for f in lis2:
        dist=util.manhattanDistance(position,f) #manhattan distance gia kathe fagito pou vrisketai sto grid
        maxi=max(dist,maxi) #telika ypologizw to biggest value
    return maxi
